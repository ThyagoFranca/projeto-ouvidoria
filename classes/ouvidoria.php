...
class ouvidoria {
	...
	public function consultar($protocolo){
		...
		$query = $this->conn->prepare('SELECT * from ouvidoria where protocolo = :prot');
		$query->execute(array(
			':prot' => $protocolo
		));
	...
	public function cadastrar() {
		$protocolo = $this->conn->query(" SELECT protocolo as ultimo from ouvidoria where ouvidoria.protocolo like concat('%/',year(now()))
		order by protocolo desc LIMIT 1");
		...
		$exec = $query->execute(array(
			...
			':telefone_numero'=>$this->telefone,
			':celular_ddd'=>$this->celular_ddd,
			':celular_numero'=>$this->celular,
